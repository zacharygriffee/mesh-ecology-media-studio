...
| `npm run operation:studio` | yes | updates existing local proof/seam/operator/control/Edge-compatible artifacts and writes `records/exports/media-current-operational-runbook.local.json` by default | `--print` | yes | runs the current operational runbook path over one project: optional `--prepare-local-fixture`, `proof:local --drill`, adjacent seam needs packet, read-only seam readiness, operator index, Edge-compatible bundle, optional `--cross-project-index`, optional `--adapter-decision rejected`, and optional `--output`; rejected adapter posture is preserved as local attention/hold with skipped observation, not malformed state; after writing the current-operation summary, refreshes inspection, control-surface, operator, Edge-compatible, and cross-project surfaces so compact output shows `operation`, `preparedLocalFixture`, proof/drill/package/swarm posture, inference-source/Venice local evidence, canonical `familyBuildout`, `familyReadiness`, `familyNextAction`, legacy `spineDiscussion`/`spineReadiness`, adjacent readiness, `familyAsks`, `familyAsksReady`, cross-project visibility, `crossProjectCurrentOperations`, `crossProjectInferenceSourceProjects`, `crossProjectVeniceEvidence`, `crossProjectInferenceFamilyAsks`, `inspectionRefreshed`, `inspectionPacket`, `controlSurfaceRefreshed`, `controlSurface`, `surfaceFreshness`, `surfaceFreshnessIssues`, summary `output`, and non-claims; `--print` includes `adjacentFamilyAskSummary` for operator and Spine repo-agent family buildout coordination; no live provider call, adjacent repo writes, Edge dispatch, Layer admission, publication authorization, production readiness, or swarm activation |
...
`veniceAssets`, and family seam/ask counts when the posture record exists.
These fields are readiness evidence and coordination pressure only; pending
asks do not become local attention by themselves. They do not prove provider
truth, Edge dispatch, Bytes materialization, Causal truth, Layer admission,
mesh publication, family seam success, or production readiness.
