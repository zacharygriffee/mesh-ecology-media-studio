...
| `npm run production:package-rework` | yes | updates existing local output artifacts | `--print` | yes | requires latest local package review to be `request_changes`, then reruns the local output lane to regenerate package review/request/handoff posture; no authority, publication, Edge call, or production readiness |
