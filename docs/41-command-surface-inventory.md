...
| `npm run seam:ready` | yes | yes | `--print` | no | read-only readiness check for Spine discussion; derives `readiness`, proof state/freshness/drill posture, adjacent seam freshness, `spineDiscussion`, and safe next action from existing local records; writes no artifacts and preserves no adjacent repo writes, Layer admission, Edge dispatch, Bytes materialization, Causal truth, swarm runtime activation, publication, or authority |
