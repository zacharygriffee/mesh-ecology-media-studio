...
| `npm run media:summary` | yes | no | `--print` | reads local refs | refreshes project status snapshot; includes provider-loop, approval-lane, production-capsule, Layer interop attention, and safe-next posture |
...
| `npm run production:local-output` | yes | yes | `--print` | yes | orchestrates existing rough-cut, review, render, export, authority-prereq, and handoff commands; no new authority, publication, Edge call, or production readiness |
