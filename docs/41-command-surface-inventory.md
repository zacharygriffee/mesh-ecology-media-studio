...
| `npm run operation:studio` | yes | updates existing local proof/seam/operator/control/Edge-compatible artifacts and writes `records/exports/media-current-operational-runbook.local.json` by default | `--print` | yes | runs the current operational runbook path over one project: optional `--prepare-local-fixture`, `proof:local --drill`, adjacent seam needs packet, read-only seam readiness, operator index, Edge-compatible bundle, optional `--cross-project-index`, and optional `--output`; after writing the current-operation summary, refreshes inspection, control-surface, operator, Edge-compatible, and cross-project surfaces so compact output shows `operation`, `preparedLocalFixture`, proof/drill/package/swarm posture, adjacent readiness, cross-project visibility, `crossProjectCurrentOperations`, `inspectionRefreshed`, `inspectionPacket`, `controlSurfaceRefreshed`, `controlSurface`, summary `output`, and non-claims; no live provider call, adjacent repo writes, Edge dispatch, Layer admission, publication authorization, production readiness, or swarm activation |
