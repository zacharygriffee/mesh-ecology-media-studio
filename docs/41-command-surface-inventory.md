...
`adjacentReady`, `adjacentAttention`, `adjacentFreshness`, and
`spineDiscussion` when the packet is present. If the packet no longer matches
current proof posture, those surfaces report it as stale local attention and
ask for `seam:needs` to be refreshed.
