...
| `npm run operation:studio` | yes | updates existing local proof/seam/operator/control/Edge-compatible artifacts and writes `records/exports/media-current-operational-runbook.local.json` by default | `--print` | yes | runs the current operational runbook path over one project: optional `--prepare-local-fixture`, `proof:local --drill`, adjacent seam needs packet, read-only seam readiness, operator index, Edge-compatible bundle, optional `--cross-project-index`, optional `--adapter-decision rejected`, and optional `--output`; rejected adapter posture is preserved as local attention/hold with skipped observation, not malformed state; after writing the current-operation summary, refreshes inspection, control-surface, operator, Edge-compatible, and cross-project surfaces so compact output shows `operation`, `preparedLocalFixture`, proof/drill/package/swarm posture, inference-source/Venice local evidence, Agent Bridge canonical future ownership, Studio media-only frontier posture, provider flexibility, canonical `familyBuildout`, `familyReadiness`, `familyNextAction`, legacy `spineDiscussion`/`spineReadiness`, adjacent readiness, `familyAsks`, `familyAsksReady`, cross-project visibility, `crossProjectCurrentOperations`, `crossProjectInferenceSourceProjects`, `crossProjectVeniceEvidence`, `crossProjectInferenceFamilyAsks`, `inspectionRefreshed`, `inspectionPacket`, `controlSurfaceRefreshed`, `controlSurface`, `surfaceFreshness`, `surfaceFreshnessIssues`, summary `output`, and non-claims; `--print` includes `adjacentFamilyAskSummary` for operator and Spine repo-agent family buildout coordination; no live provider call, adjacent repo writes, Edge dispatch, Layer admission, publication authorization, production readiness, or swarm activation |
...
| `npm run seam:needs` | yes | yes | `--print` | yes | reads the latest local proof, operator index, Edge-compatible bundle, source-pressure evidence, and optional inference-source posture; writes `media.studio_adjacent_seam_needs_packet.local.v1` as a discussion-only declaration for the operator and Spine repo agent as family seam buildout coordination; compact output shows `declaration`, canonical `familyBuildout`, legacy `spineDiscussion`, `proof`, `proofFreshness`, `proofDrill`, `drillAttentionReasons`, `adjacentNeeds`, `adjacentReady`, `adjacentAttention`, `inferenceSourceFamilyAsks`, `inferenceSourcePendingFamilySeams`, `inferenceSourceVeniceEvidence`, `agentBridgeCanonical`, `studioFrontier`, `llmFrontier`, and `providerFlex`; operator/Edge/cross-project summaries also report `adjacentFreshness` and treat stale packets, including changed proof drill attention reasons, as local attention; inference-source asks are additive coordination pressure and do not increase adjacent attention; no adjacent repo writes, Layer admission, Edge queue/dispatch, Bytes materialization, Causal truth, result acceptance, storage selection, publication, or authority |
...
| `npm run inference:source-posture` | yes | yes | `--print` | yes | writes `media.studio_inference_source_posture.local.v1` from existing local records; classifies operator-supplied, Studio provider-adapter, Edge agent-seat, local inference, Agent Bridge/BYO-AI, and mesh-v0-2 pub/rat lanes; Venice is local `studio_provider_adapter` evidence when present; records Agent Bridge/BYO-AI as future canonical provider-normalization owner, Studio as media-inference frontier evidence only, `generalLlmProviderFrontier=false`, and provider modeling as a stable common envelope plus provider-specific config blocks; no provider call, provider truth, Edge dispatch, Bytes materialization, Causal truth, Layer admission, mesh publication, family seam proof, or production readiness |
...
| `npm run inspect:index` | yes | no | `--json` | reads local refs | no committed output; reports current-operation summaries as an explicit local count/ref group when present |
...
| `npm run control:surface` | yes | yes | `--print` | yes | runtime projection uses fresh timestamp; includes current-operation summary refs and a local preview action when present, with no surface request, UI contract, mesh write, or authority claim |
| `npm run edge:compat` | yes | yes | `--print` | yes | runtime bundle uses fresh timestamp; includes production-capsule, production-bundle, rough-cut, render/export candidate, authority-prereq, local package posture, local proof rehearsal posture, current operation summary ref, current-operation family ask counts, swarm seam posture, and Layer interop source refs/attention when present; compact output uses `proofFreshness`, `proofStaleReasons`, `proofDrill`, `drillAttentionReasons`, `currentOperation`, `currentOperationFamilyAsks`, `currentOperationFamilyAsksReady`, `currentOperationPath`, `adjacentStaleReasons`, canonical `familyBuildout`, `familyReadiness`, `familyNextAction`, one selected `nextAction`, plus `packageNextAction`, `proofNextAction`, legacy `spineNextAction`, `swarmNextAction`, and legacy `spine*` no-authority/no-production flags; no Edge queue, dispatch, approval, swarm activation, or runtime verification |
| `npm run operator:index` | yes | yes | `--print` | yes | runtime index uses fresh timestamp; includes provider-loop, production-capsule, production-bundle, rough-cut, local package posture, local proof rehearsal posture, current operation summary ref, current-operation family ask counts, swarm seam posture, and Layer interop refs/attention when present; compact output uses `proofFreshness`, `proofStaleReasons`, `proofDrill`, `drillAttentionReasons`, `currentOperation`, `currentOperationFamilyAsks`, `currentOperationFamilyAsksReady`, `currentOperationPath`, `adjacentStaleReasons`, canonical `familyBuildout`, `familyReadiness`, `familyNextAction`, one selected `nextAction`, plus `packageNextAction`, `proofNextAction`, legacy `spineNextAction`, `swarmNextAction`, and legacy `spine*` no-authority/no-production flags |
...
| `npm run operator:cross-project-index` | yes | yes | `--print` | yes | preserves existing `createdAt` for the same output/index id; can surface provider-loop status/decision refs, Layer interop attention, local package posture, source-pressure adapter summary, local proof rehearsal posture, inference-source posture, adjacent seam readiness, and swarm seam posture from explicit operator-index refs, with health fallback for package/swarm fields; compact output includes `localPackageComplete`, `localPackageAttention`, `localProofReady`, `localProofAttention`, `localProofFresh`, `localProofStale`, `inferenceSourceProjects`, `inferenceSourceEvidence`, `veniceEvidence`, `inferenceFamilyAsks`, `agentBridgeCanonical`, `studioFrontier`, `llmFrontier`, `providerFlex`, `spineReady`, `spineAttention`, `spineFresh`, `spineStale`, `spineInherited`, `swarmReady`, `swarmAttention`, `swarmProof=false`, and `activation=false`; per-project local proof attention lines include `drillAttentionReasons`; per-project adjacent seam attention lines include `adjacentFreshness`, `readinessFreshness`, `staleReasons`, and `drillAttentionReasons`; pending inference-source family asks are not local attention or runtime authority |
...
`operator:index`, `edge:compat`, `seam:needs`, `operator:cross-project-index`,
and `operation:studio` surface compact `inferenceSource`, `veniceEvidence`,
`veniceAssets`, family seam/ask counts, `agentBridgeCanonical`,
`studioFrontier`, `llmFrontier`, and `providerFlex` when the posture record
exists. These fields are readiness evidence and coordination pressure only;
pending asks do not become local attention by themselves. Agent Bridge/BYO-AI
is the future canonical provider-normalization owner; Studio is only
frontiering media-inference evidence and must not frontier general LLM provider
provision. They do not prove provider truth, Edge dispatch, Bytes
materialization, Causal truth, Layer admission, mesh publication, family seam
...
examples/card-to-candidate --prepare-local-fixture --cross-project-index`; it
prepares the bundled example fixture, then performs the local proof drill,
adjacent seam needs declaration, read-only seam readiness check, operator index
refresh, Edge-compatible bundle refresh, and one-project cross-project operator
index refresh. Omit `--prepare-local-fixture` when a project already has current
local package evidence, and omit `--cross-project-index` when aggregate
visibility is not needed. For one consolidated local proof rehearsal without
adjacent seam declaration, run `npm run proof:local -- --project-dir
...
examples/card-to-candidate` to inspect the derived family buildout readiness
without writing another artifact. The packet and readiness check are for the
operator and Spine repo agent as family seam buildout coordination before any
Layer, Edge, Bytes, Causal, or other family implementation. Spine is not runtime
routing or dispatch. `seam:ready` compact output includes `adjacentPackets`,
...
`adjacentAttention`, `adjacentFreshness`, `adjacentStaleReasons`, canonical
`familyBuildout`, and legacy `spineDiscussion` when the packet is present;
`operator:index` and `edge:compat` also print canonical `familyReadiness` and
`familyNextAction` alongside legacy `spineReadiness`, `spineNextAction`, and
`spine*` no-authority/no-production flags, while
`operator:cross-project-index` aggregates `familyReady` and `familyAttention`
alongside legacy `spineReady` and `spineAttention` plus freshness counts. Its
per-project adjacent seam attention line also names `adjacentFreshness`,
`readinessFreshness`, `staleReasons`, and `drillAttentionReasons`. If the packet
no longer matches current proof posture,
