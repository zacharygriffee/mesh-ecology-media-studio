...
test('inference-source posture classifies Venice dry-run evidence without live provider', async () => {
  ...
  assert.equal(posture.canonicalProviderOwner, 'agent_bridge_byo_ai')
  assert.equal(posture.studioRole, 'media_inference_frontier_evidence')
  assert.equal(posture.generalLlmProviderFrontier, false)
  assert.equal(posture.providerFlexibilityPosture.strategy, 'stable_common_envelope_with_provider_specific_config')
  assert.equal(posture.providerFlexibilityPosture.lowestCommonDenominator, false)
  assert.equal(posture.providerFlexibilityPosture.mediaSpecificFrontier, true)
  assert.equal(posture.familyBuildoutAsks.length, 6)
  assert.equal(posture.familyBuildoutAsks.every((ask) => ask.status === 'needed'), true)
  assert.ok(posture.familyBuildoutAsks.find((ask) => ask.ownerRepo === 'agent-bridge-byo-ai-planned').summary.includes('canonical provider normalization'))
  assert.ok(posture.familyBuildoutAsks.find((ask) => ask.ownerRepo === 'mesh-ecology-edge').summary.includes('flexible provider config'))
  assert.ok(posture.familyBuildoutAsks.find((ask) => ask.ownerRepo === 'mesh-ecology-bytes').summary.includes('large outputs'))
  assert.ok(posture.familyBuildoutAsks.find((ask) => ask.ownerRepo === 'causal-substrate').summary.includes('character identity'))
  assert.equal(posture.summary.seamProof, false)
  assert.equal(posture.summary.familySeamSuccess, false)
  assert.equal(posture.summary.canonicalProviderOwner, 'agent_bridge_byo_ai')
  assert.equal(posture.summary.generalLlmProviderFrontier, false)
  assert.equal(posture.summary.providerLowestCommonDenominator, false)
  assert.equal(posture.nonClaims.liveProviderCalledByPosture, false)
  assert.equal(posture.nonClaims.agentBridgeCanonicalized, false)
...
test('operator and Edge-compatible surfaces carry inference-source posture refs without authority claims', async () => {
  ...
  assert.equal(index.summary.inferenceSourceCanonicalProviderOwner, 'agent_bridge_byo_ai')
  assert.equal(index.summary.inferenceSourceStudioRole, 'media_inference_frontier_evidence')
  assert.equal(index.summary.inferenceSourceGeneralLlmProviderFrontier, false)
  assert.equal(index.summary.inferenceSourceProviderFlexibilityStrategy, 'stable_common_envelope_with_provider_specific_config')
  assert.equal(index.summary.inferenceSourceProviderLowestCommonDenominator, false)
  assert.equal(index.summary.inferenceSourceMediaSpecificFrontier, true)
  ...
  assert.equal(crossProjectResult.index.summary.inferenceSourceAgentBridgeCanonical, 1)
  assert.equal(crossProjectResult.index.summary.inferenceSourceStudioFrontier, 1)
  assert.equal(crossProjectResult.index.summary.inferenceSourceGeneralLlmProviderFrontier, 0)
  assert.equal(crossProjectResult.index.summary.inferenceSourceProviderLowestCommonDenominator, 0)
  assert.equal(crossProjectResult.index.summary.inferenceSourceProviderFlexibilityStrategy, 'stable_common_envelope_with_provider_specific_config')
  ...
  assert.equal(crossProjectResult.index.projectSummaries[0].inferenceSourcePosture.canonicalProviderOwner, 'agent_bridge_byo_ai')
  assert.equal(crossProjectResult.index.projectSummaries[0].inferenceSourcePosture.studioRole, 'media_inference_frontier_evidence')
  assert.equal(crossProjectResult.index.projectSummaries[0].inferenceSourcePosture.generalLlmProviderFrontier, false)
  assert.equal(crossProjectResult.index.projectSummaries[0].inferenceSourcePosture.providerLowestCommonDenominator, false)
  ...
  assert.ok(crossProjectLines[0].includes('agentBridgeCanonical=1'))
  assert.ok(crossProjectLines[0].includes('studioFrontier=1'))
  assert.ok(crossProjectLines[0].includes('llmFrontier=0'))
...
test('current operational command completes local proof through review surfaces', async () => {
  ...
  assert.equal(output.result.operation.familyBuildoutCoordination, 'required')
  assert.equal(output.result.operation.adjacentReadiness, 'ready_for_spine_discussion')
  assert.equal(output.result.operation.familyBuildoutReadiness, 'ready_for_spine_discussion')
  assert.match(output.result.operation.familyBuildoutNextAction, /family seam buildout coordination/)
  ...
  assert.equal(output.result.operation.adjacentFamilyAskSummary.familyBuildoutCoordination, 'required')
  ...
  assert.equal(output.result.operation.crossProjectSummary.familyReady, 1)
  ...
  assert.equal(output.result.operation.outputs.controlSurfaceProjection, 'records/exports/media-control-surface-projection.local.json')
  assert.equal(output.result.controlSurface.output, 'records/exports/media-control-surface-projection.local.json')
  assert.equal(output.result.controlSurface.projection.observationRefs.currentOperationSummary.path, 'records/exports/media-current-operational-runbook.local.json')
  assert.equal(output.result.controlSurface.projection.observationRefs.currentOperationSummary.schema, 'studio-current-operational-runbook')
  assert.equal(output.result.controlSurface.projection.observationRefs.currentOperationSummary.localOnly, true)
  assert.ok(output.result.controlSurface.projection.actions.some((entry) => entry.actionId === 'current-operation' && entry.surfaceRequest === false))
  ...
  assert.equal(output.result.crossProject.index.summary.familyBuildoutReadinessReady, 1)
  ...
  assert.ok(line.includes('familyBuildout=required'))
  assert.ok(line.includes('familyReadiness=ready_for_spine_discussion'))
  ...
  assert.ok(line.includes('crossProjectFamilyReady=1'))
  ...
  assert.ok(line.includes('familyNextAction=Discuss these adjacent seam needs with the operator and Spine repo agent as family seam buildout coordination'))
})
...
test('adjacent seam needs packet declares ready proof for family buildout coordination', async () => {
  const dir = await createLocalProofFixtureProject()
  ...
  assert.deepEqual(schema.properties.familyBuildoutCoordination.enum, ['required', 'not-ready', 'absent'])
  assert.equal(packet.declarationStatus, 'ready_for_spine_discussion')
  assert.equal(packet.spineDiscussion, 'required')
  assert.equal(packet.familyBuildoutCoordination, 'required')
  assert.equal(packet.summary.familyBuildoutCoordination, 'required')
  ...
  assert.ok(line.includes('proofDrill=passed'))
  const spineRow = packet.adjacentDiscussionRows.find((row) => row.ownerRepo === 'mesh-ecology-spine')
  assert.equal(spineRow.discussionKind, 'family-seam-buildout-coordination')
  assert.match(spineRow.requestedReview, /repo-family build coordination/)
  assert.doesNotMatch(spineRow.requestedReview, /routing any family work/)
  assert.doesNotMatch(spineRow.nextAction, /route/)
...
test('adjacent seam needs packet carries inference-source family asks as coordination pressure', async () => {
  ...
  assert.equal(packet.summary.inferenceSourceCanonicalProviderOwner, 'agent_bridge_byo_ai')
  assert.equal(packet.summary.inferenceSourceStudioRole, 'media_inference_frontier_evidence')
  assert.equal(packet.summary.inferenceSourceGeneralLlmProviderFrontier, false)
  assert.equal(packet.summary.inferenceSourceProviderFlexibilityStrategy, 'stable_common_envelope_with_provider_specific_config')
  assert.equal(packet.summary.inferenceSourceProviderLowestCommonDenominator, false)
  ...
  assert.equal(packet.inferenceSourceFamilyAskRows.every((row) => row.canonicalProviderOwner === 'agent_bridge_byo_ai'), true)
  assert.equal(packet.inferenceSourceFamilyAskRows.every((row) => row.studioRole === 'media_inference_frontier_evidence'), true)
  ...
  assert.ok(line.includes('agentBridgeCanonical=true'))
  assert.ok(line.includes('studioFrontier=media_inference_frontier_evidence'))
  assert.ok(line.includes('llmFrontier=false'))
  assert.equal(validateRequiredRecord(packet), true)
})
...
test('adjacent seam needs surface through operator Edge and cross-project views', async () => {
  ...
  assert.equal(operatorResult.index.adjacentSeamNeedsSummary.familyBuildoutCoordination, 'required')
  ...
  assert.equal(operatorResult.index.adjacentSeamReadiness.familyBuildoutReadiness, 'ready_for_spine_discussion')
  ...
  assert.equal(operatorResult.index.summary.adjacentSeamNeedsFamilyBuildoutCoordination, 'required')
  assert.equal(operatorResult.index.summary.adjacentSeamReadinessFamilyBuildoutReadiness, 'ready_for_spine_discussion')
  ...
  assert.ok(operatorLines.find((entry) => entry.startsWith('operator packet index:')).includes('familyBuildout=required'))
  assert.ok(operatorLines.find((entry) => entry.startsWith('operator packet index:')).includes('familyReadiness=ready_for_spine_discussion'))
  assert.ok(operatorLines.find((entry) => entry.startsWith('operator packet index:')).includes('familyNextAction=Discuss these adjacent seam needs with the operator and Spine repo agent as family seam buildout coordination'))
  ...
  assert.ok(operatorLines.find((entry) => entry.startsWith('operator packet index:')).includes('spineNextAction=Discuss these adjacent seam needs with the operator and Spine repo agent as family seam buildout coordination'))
  ...
  assert.equal(edgeResult.bundle.adjacentSeamNeedsSummary.familyBuildoutCoordination, 'required')
  ...
  assert.equal(edgeResult.bundle.adjacentSeamReadiness.familyBuildoutReadiness, 'ready_for_spine_discussion')
  ...
  assert.ok(edgeLines.find((entry) => entry.startsWith('edge source refs:')).includes('familyBuildout=required'))
  assert.ok(edgeLines.find((entry) => entry.startsWith('edge source refs:')).includes('familyReadiness=ready_for_spine_discussion'))
  assert.ok(edgeLines.find((entry) => entry.startsWith('edge source refs:')).includes('familyNextAction=Discuss these adjacent seam needs with the operator and Spine repo agent as family seam buildout coordination'))
  assert.ok(edgeLines.find((entry) => entry.startsWith('edge source refs:')).includes('spineReadiness=ready_for_spine_discussion'))
  assert.ok(edgeLines.find((entry) => entry.startsWith('edge source refs:')).includes('spineNextAction=Discuss these adjacent seam needs with the operator and Spine repo agent as family seam buildout coordination'))
  ...
  assert.equal(crossProjectResult.index.summary.familyBuildoutRequired, 1)
  assert.equal(crossProjectResult.index.summary.spineReadinessReady, 1)
  assert.equal(crossProjectResult.index.summary.familyBuildoutReadinessReady, 1)
  assert.equal(crossProjectResult.index.summary.spineReadinessAttention, 0)
  assert.equal(crossProjectResult.index.summary.familyBuildoutReadinessAttention, 0)
  assert.equal(crossProjectResult.index.summary.spineReadinessFresh, 1)
  assert.equal(crossProjectResult.index.summary.familyBuildoutReadinessFresh, 1)
  assert.equal(crossProjectResult.index.summary.spineReadinessStale, 0)
  assert.equal(crossProjectResult.index.summary.familyBuildoutReadinessStale, 0)
  assert.equal(crossProjectResult.index.summary.spineReadinessInherited, 0)
  assert.equal(crossProjectResult.index.summary.familyBuildoutReadinessInherited, 0)
  ...
  assert.ok(crossProjectLines[0].includes('familyBuildout=1'))
  assert.ok(crossProjectLines[0].includes('familyReady=1'))
  assert.ok(crossProjectLines[0].includes('familyAttention=0'))
...
test('adjacent seam readiness command reports ready missing and stale local states', async () => {
  ...
  assert.equal(readyResult.readiness.familyBuildoutCoordination, 'required')
  assert.equal(readyResult.readiness.familyBuildoutReadiness, 'ready_for_spine_discussion')
  assert.match(readyResult.readiness.familyBuildoutNextAction, /family seam buildout coordination/)
  ...
  assert.ok(compact.includes('familyBuildout=required'))
  assert.ok(compact.includes('familyReadiness=ready_for_spine_discussion'))
  assert.ok(compact.includes('familyNextAction=Discuss these adjacent seam needs with the operator and Spine repo agent as family seam buildout coordination'))
...
test('control surface projection maps Packs planes without UI contract', async () => {
  ...
  assert.ok(projection.actions.some((entry) => entry.actionId === 'current-operation' && entry.writesMesh === false))
