...
export async function runCurrentOperationalRunbook({
  ...
  if (crossProjectIndex) {
    ...
    operation.crossProjectSummary = {
      ...
      inferenceSourceProjects: crossProject.index.summary.inferenceSourceProjects ?? 0,
      inferenceSourceEvidence: crossProject.index.summary.inferenceSourceEvidence ?? 0,
      inferenceSourceVeniceEvidence: crossProject.index.summary.inferenceSourceVeniceEvidence ?? 0,
      inferenceSourceFamilyAsks: crossProject.index.summary.inferenceSourceFamilyAsks ?? 0,
      inferenceSourceAgentBridgeCanonical: crossProject.index.summary.inferenceSourceAgentBridgeCanonical ?? 0,
      inferenceSourceStudioFrontier: crossProject.index.summary.inferenceSourceStudioFrontier ?? 0,
      inferenceSourceGeneralLlmProviderFrontier: crossProject.index.summary.inferenceSourceGeneralLlmProviderFrontier ?? 0,
      swarmProof: false,
      activation: false,
      localOnly: true,
...
export function createCurrentOperationalRunbookSummary({
  ...
  return {
    ...
    inferenceSourceState: operatorIndex.index.summary.inferenceSourceState ?? 'not_evidenced',
    inferenceSourcePostures: operatorIndex.index.summary.inferenceSourcePostures ?? 0,
    veniceEvidence: operatorIndex.index.summary.inferenceSourceVeniceEvidence === true,
    veniceGeneratedAssets: operatorIndex.index.summary.inferenceSourceVeniceGeneratedAssets ?? 0,
    inferenceSourceFamilySeams: operatorIndex.index.summary.inferenceSourceFamilySeams ?? 0,
    inferenceSourceCanonicalProviderOwner: operatorIndex.index.summary.inferenceSourceCanonicalProviderOwner ?? 'agent_bridge_byo_ai',
    inferenceSourceStudioRole: operatorIndex.index.summary.inferenceSourceStudioRole ?? 'media_inference_frontier_evidence',
    inferenceSourceGeneralLlmProviderFrontier: operatorIndex.index.summary.inferenceSourceGeneralLlmProviderFrontier === true,
    inferenceSourceProviderFlexibilityStrategy: operatorIndex.index.summary.inferenceSourceProviderFlexibilityStrategy ?? 'stable_common_envelope_with_provider_specific_config',
...
export function formatCurrentOperationalRunbook(operation) {
  return [
    ...
    `inferenceSource=${operation.inferenceSourceState}`,
    `veniceEvidence=${operation.veniceEvidence === true}`,
    `veniceAssets=${operation.veniceGeneratedAssets ?? 0}`,
    `agentBridgeCanonical=${operation.inferenceSourceCanonicalProviderOwner === 'agent_bridge_byo_ai'}`,
    `studioFrontier=${operation.inferenceSourceStudioRole ?? 'media_inference_frontier_evidence'}`,
    `llmFrontier=${operation.inferenceSourceGeneralLlmProviderFrontier === true}`,
    `providerFlex=${operation.inferenceSourceProviderFlexibilityStrategy ?? 'stable_common_envelope_with_provider_specific_config'}`,
    ...
    `crossProjectInferenceSourceProjects=${operation.crossProjectSummary?.inferenceSourceProjects ?? 0}`,
    `crossProjectVeniceEvidence=${operation.crossProjectSummary?.inferenceSourceVeniceEvidence ?? 0}`,
    `crossProjectInferenceFamilyAsks=${operation.crossProjectSummary?.inferenceSourceFamilyAsks ?? 0}`,
    `crossProjectAgentBridgeCanonical=${operation.crossProjectSummary?.inferenceSourceAgentBridgeCanonical ?? 0}`,
    `crossProjectStudioFrontier=${operation.crossProjectSummary?.inferenceSourceStudioFrontier ?? 0}`,
    `crossProjectLlmFrontier=${operation.crossProjectSummary?.inferenceSourceGeneralLlmProviderFrontier ?? 0}`,
