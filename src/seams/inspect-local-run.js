...
function nameForSchema(schema, relativePath) {
  const schemaNames = {
    ...
    'media.project_health.local.v1': `projectHealth:${path.basename(relativePath, '.json')}`,
    ...
    [artifactKinds.mediaProductionBundleLocal]: `productionBundle:${path.basename(relativePath, '.json')}`,
    [artifactKinds.mediaRoughCutCapsuleLocal]: `roughCutCapsule:${path.basename(relativePath, '.json')}`,
...
function kindForSchema(schema) {
  const schemaKinds = {
    ...
    'media.project_health.local.v1': 'media-project-health',
