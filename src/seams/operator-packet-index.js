...
export async function writeOperatorPacketIndex({
  ...
  const index = {
    ...
    summary: {
      ...
      localProofDrillStatus: localProofRehearsalSummary.drillStatus,
      localProofDrillChecks: localProofRehearsalSummary.drillChecks,
      localProofDrillAttention: localProofRehearsalSummary.drillAttention,
      localProofDrillAttentionReasons: localProofRehearsalSummary.drillAttentionReasons,
      ...
      adjacentSeamNeedsFamilyBuildoutCoordination: adjacentSeamNeedsSummary.familyBuildoutCoordination ?? adjacentSeamNeedsSummary.spineDiscussion,
      ...
      currentOperationFamilyAskState: currentOperationSummary.familyAskState,
      currentOperationFamilyAsks: currentOperationSummary.familyAsks,
      currentOperationFamilyAsksReady: currentOperationSummary.familyAsksReady,
      currentOperationFamilyAsksAttention: currentOperationSummary.familyAsksAttention,
...
function formatOperatorPacketIndexSummary(index, output) {
  ...
  return [
    ...
    `proofStaleReasons=${formatStaleReasons(summary.localProofStaleReasons)}`,
    `proofDrill=${summary.localProofDrillStatus ?? 'absent'}`,
    `drillAttention=${summary.localProofDrillAttention ?? 0}`,
    `drillAttentionReasons=${formatStaleReasons(summary.localProofDrillAttentionReasons)}`,
    ...
    `spineDurableAppend=${index.adjacentSeamReadiness?.durableAppend === true}`,
    `spineEdgeQueueAction=${index.adjacentSeamReadiness?.edgeQueueAction === true}`,
    ...
    `currentOperationFamilyAsks=${summary.currentOperationFamilyAsks ?? 0}`,
    `currentOperationFamilyAsksReady=${summary.currentOperationFamilyAsksReady ?? 0}`,
...
function formatLocalProofRehearsalSummary(summary) {
  return [
    ...
    `proofStaleReasons=${formatStaleReasons(summary.staleReasons)}`,
    `proofDrill=${summary.drillStatus}`,
    `drillAttention=${summary.drillAttention}`,
    `drillAttentionReasons=${formatStaleReasons(summary.drillAttentionReasons)}`,
...
async function readCurrentOperationSummary(root, projectId) {
  ...
  if (readResult.missing) {
    return {
      ...
      familyAskState: 'absent',
      familyAsks: 0,
      familyAsksReady: 0,
      familyAsksAttention: 0,
  ...
  return {
    ...
    familyAskState: summary.adjacentFamilyAskSummary?.state ?? 'unknown',
    familyAsks: summary.adjacentFamilyAskSummary?.adjacentNeeds ?? 0,
    familyAsksReady: summary.adjacentFamilyAskSummary?.adjacentReady ?? 0,
    familyAsksAttention: summary.adjacentFamilyAskSummary?.adjacentAttention ?? 0,
